...
*/
/**
 * @file array_separators.c
 * @brief The file containing the array_separators function
 */

#include "../../include/myshell.h"

/**
 * @brief Check if the character is not a separator
 * @param c The character
 * @param sep The list of separators
 * @return <b>int</b> <u>1</u> if the character is not a separator,
 * <u>0</u> otherwise
 */
    ...
    char const *separator)
{
    for (int i = 0; str[i] != '\0'; i++) {
        if ((my_char_is_not_a_separator(str[i], separator)
        || char_is_protected((char *)str, i)) && *sep == 1) {
            *word = *word + 1;
            *sep = 0;
        }
        if (my_char_is_not_a_separator(str[i], separator) == 0 &&
            char_is_protected((char *)str, i) == 0)
            *sep = 1;
    }
}
    ...
    char const *separator)
{
    ...
    && (my_char_is_not_a_separator(str[i], separator) ||
    char_is_protected((char *)str, i)); i++)
    ...
    int *which_lettre, int *nbr_lettre, char const *separator)
{
    ...
    && char_is_protected((char *)str, *which_lettre) == 0;
...
static char *nbr_sep(char *str, char const *separator)
{
    ...
    for (int i = 0; my_char_is_not_a_separator(str[i], separator) == 0
    && char_is_protected((char *)str, i) == 0; i++)
        nbr_sep++;
    return &str[nbr_sep];
}
